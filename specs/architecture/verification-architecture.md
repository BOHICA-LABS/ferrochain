...
changelog:
  ...
  - "2.44 (D-356/DC-07/2026-09-07, architect): F-PDC07-03 — 10 panel-VP Module cells repointed to canonical SPA component path form: spa_components::run_inspector (VP-2.24.004-A/B), spa_components::checkpoint_panel (VP-2.24.005-A/B), spa_components::hitl_panel (VP-2.24.006-A/B), spa_components::budget_panel (VP-2.24.007-A/B), spa_components::guardrail_panel (VP-2.24.008-A/B). CORRECTION: v2.41 (DC-02) claimed these were SPA panel modules but used console::* Rust module notation — non-canonical for SPA components. Canonical convention: spa_components::<component_name> path form. Census UNCHANGED: 41 total. input-hash unchanged (inputs did not change)."
...
| VP-2.24.004-A | BC-2.24.004 | DI-014 | `spa_components::run_inspector` | integration | 3 | P1 |
| VP-2.24.004-B | BC-2.24.004 | DI-014 | `spa_components::run_inspector` | integration | 3 | P1 |
| VP-2.24.005-A | BC-2.24.005 | DI-004 | `spa_components::checkpoint_panel` | integration | 3 | P1 |
| VP-2.24.005-B | BC-2.24.005 | DI-004 | `spa_components::checkpoint_panel` | integration | 3 | P1 |
| VP-2.24.006-A | BC-2.24.006 | DI-003 | `spa_components::hitl_panel` | integration | 3 | P1 |
| VP-2.24.006-B | BC-2.24.006 | DI-003 | `spa_components::hitl_panel` | integration | 3 | P1 |
| VP-2.24.007-A | BC-2.24.007 | DI-014 | `spa_components::budget_panel` | unit | 3 | P1 |
| VP-2.24.007-B | BC-2.24.007 | DI-014 | `spa_components::budget_panel` | integration | 3 | P1 |
| VP-2.24.008-A | BC-2.24.008 | DI-012 | `spa_components::guardrail_panel` | integration | 3 | P1 |
| VP-2.24.008-B | BC-2.24.008 | DI-014 | `spa_components::guardrail_panel` | unit | 3 | P1 |

**Total: 43 VPs — 7 P0 / 36 P1 | Tool breakdown: Kani ×10, proptest ×10, integration ×14, unit ×8, compile-fail ×1**
...
> **D-356 adversary fix DC-02 (2026-09-07, architect).** F-PDC02-03: VP-2.24.002-A/B module repointed `console::ring_buffer` → `console::span_exporter` (ADR-031 Decision 5 canonical; `console::ring_buffer` was a non-canonical invention; canonical module is `console::span_exporter` — Boundary module hosting `RingBuffer<SpanData>`, per ADR-031 Decision 5 purity table). F-PDC02-02: VP-2.24.004-008 A/B module columns corrected from `console::server` to SPA panel modules per respective BCs: `console::run_inspector` (004-A/B), `console::checkpoint_panel` (005-A/B), `console::hitl_panel` (006-A/B), `console::budget_panel` (007-A/B), `console::guardrail_panel` (008-A/B). Tool census UNCHANGED. See companion fixes in api-surface.md and ADR-031 (F-PDC02-05 mandatory `debug_route_key`). *[DC-08 records sweep: `debug_api_key` → `debug_route_key` for DC-07-rename consistency.]*
...
> **D-356 adversary fix DC-07 (2026-09-07, architect).** F-PDC07-03: 10 panel-VP Module cells repointed to canonical SPA component path form — `spa_components::run_inspector` (VP-2.24.004-A/B), `spa_components::checkpoint_panel` (VP-2.24.005-A/B), `spa_components::hitl_panel` (VP-2.24.006-A/B), `spa_components::budget_panel` (VP-2.24.007-A/B), `spa_components::guardrail_panel` (VP-2.24.008-A/B). CORRECTION: v2.41 (DC-02) described these as "SPA panel modules" but used `console::*` Rust module notation — non-canonical for SPA components. Canonical form: `spa_components::<component_name>` path form (matches VP-INDEX.md preamble SPA convention note). Census UNCHANGED: 41 total.
...
corrected; `server::run_read_handler` is not a separate module — F-PDC48-02/DC-48).
...
VP-019 integration test (BC-2.04.011 TV-002 + EC-001).
...
`retention_frontier_frac in 0.0..=1.0`, `promote_frac in 0.0..=0.5` (derives `TrajectoryRetentionPolicy`).
Oracle: `r.step_idx >= retention_frontier || promoted.contains(&r.step_idx)` (semantic definition,
independent code path from `is_eligible`). Negative mutation test: `compact_in_memory_buggy`
uses `<=` instead of `<` for frontier; asserts buggy result diverges from correct result (round-50
harness rework per F-P2A209-02; independent oracle confirmed non-tautological).
...
Two test functions: encrypted-path (asserts NOT plaintext) + plaintext-baseline (asserts IS plaintext,
confirming the encryption assertion is non-vacuous). `GuardrailResult::Fail` constructed as struct
variant `Fail { reason: ..., severity: ... }` per DC-57 struct-variant discipline.
See `vp-2.11.007-b-guardrail-journal-encryption-at-rest.md` §Proof Harness Skeleton.

DI anchor: DI-012 (Guardrail Coverage at Ingress Boundaries — durable audit log must not
be readable from raw storage; encryption at rest is part of the CAP-047 audit integrity guarantee).
...
|---------|------|--------|----------|--------|
| 2.56 | 2026-09-10 | architect | DC-62/2026-09-10 | VP-2.11.007-B minted — GuardrailJournal encryption-at-rest integration P1 (BC-2.11.007 {INV-005} + BC-2.04.007 {INV-006}; DI-012; checkpoint::encryption; pregolya-checkpoint; Phase 3). Preamble twenty-two→twenty-three; Forty-two→Forty-three. Committed VP Obligations table row added. VP total 42→43, P1 35→36, integration ×13→×14. §P1 catalog entry added. Census 42→43. BC-2.04.007.md added to inputs. |
| 2.55 | 2026-09-09 | architect | DC-52/A-03/2026-09-09 | A-03 (LOW) — VP-2.11.007-A §P0 Must-Prove prose block: BC-2.06.001 §PC-002 → BC-2.06.001 {PC-002} (ADR-027 stable clause anchor form; records-straggler sweep). Census UNCHANGED: 42 total. |
| 2.54 | 2026-09-09 | architect | D-356/DC-48/F-PDC48-02/2026-09-09 | F-PDC48-02 (MED) — phantom `server::run_read_handler` replaced with canonical `server::handlers` in VP-2.11.007-A NOTE block. server::run_read_handler is not a separate module (F-PDC48-02/DC-48). Census UNCHANGED: 42 total. |
| 2.53 | 2026-09-09 | architect | D-356/DC-46/2026-09-09 | DC-46 gate-backlog fix — 13 `spa/components/` module path forms converted to `spa_components::` namespace::module form across VP catalog table and P0/P1 lists (verify-module-canonicality.sh canonical pattern). Census UNCHANGED: 42 total. |
...
| 2.44 | 2026-09-07 | architect | D-356/DC-07/2026-09-07 | F-PDC07-03 — 10 panel-VP Module cells repointed to canonical SPA component path form: spa_components::run_inspector (VP-2.24.004-A/B), spa_components::checkpoint_panel (VP-2.24.005-A/B), spa_components::hitl_panel (VP-2.24.006-A/B), spa_components::budget_panel (VP-2.24.007-A/B), spa_components::guardrail_panel (VP-2.24.008-A/B). CORRECTION: v2.41 (DC-02) used console::* Rust module notation — non-canonical for SPA components. Canonical convention: spa_components::<component_name> path form. Census UNCHANGED: 41 total. |
