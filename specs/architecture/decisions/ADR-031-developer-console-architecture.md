...
| Budget policy decisions (Allow/Escalate/Deny) | `evidence_journal?` (BC-2.12.003 {PC-013}) — append-only record of `PolicyDecision` per BudgetPolicy evaluation; owned by BC-2.10.002 | `GET /threads/{id}/runs/{run_id}` (terminal-status runs only) |
| Guardrail evaluation results (Pass/Fail/Transform) | `guardrail_journal?` (BC-2.12.003 {PC-013}) — durable per-run `GuardrailJournal` record; governed by BC-2.11.007; entity defined in entities-server.md §GuardrailJournal; DC-33 human-authorized core amendment | `GET /threads/{id}/runs/{run_id}` (terminal-status runs only) |
