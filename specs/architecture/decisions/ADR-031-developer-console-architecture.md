...
input-hash: "e3853f1"
changelog:
  - "1.18 (DC-51/F-PDC51-01/2026-09-09, architect): F-PDC51-01 (MED) — reconcile with dependency-graph.md §Edge Table: the pregolya-console→pregolya-server edge is now present, resolving §Decision 7 ('This is the already-asserted console→server direction in dependency-graph.md') and DC-10 delta note ('already the asserted direction in dependency-graph.md'). Both console→server citations now resolve correctly. No body wording change required; version bump records reconciliation. input-hash unchanged (inputs did not change). [Prose corrected in-place by F-PDC54-01/DC-54: removed false console→core over-claim and false dependency-graph.md input-hash note from original draft.]"
