...
| {EC-002} | `GET /threads/{id}/state?checkpoint_id=<id>` returns 404 (checkpoint evicted or compacted) | Panel shows "checkpoint no longer available" message for that entry; does not crash |
