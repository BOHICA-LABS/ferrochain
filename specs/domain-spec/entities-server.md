...
- **Journal projections (terminal-status only):** `evidence_journal?` records `PolicyDecision` budget outcomes; `guardrail_journal?` records `GuardrailResult` guardrail outcomes — MUST NOT be conflated. Both fields are `None` in non-terminal states (`queued`, `in_progress`, `interrupted`); projected on terminal-status runs (`completed | failed | cancelled | summary_halt`). Authority: BC-2.10.002, BC-2.11.007, BC-2.12.003 {PC-013}.
